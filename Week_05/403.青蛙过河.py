...
class Solution:
    def canCross(self, stones: List[int]) -> bool:
        ...
        stoneSet = set(stones)  # 第一次有这个意识，但没这样做。因为我以为影响不大，结果影响很大6500ms->280ms。
        usedSteps = collections.defaultdict(set)
        usedSteps[1].add(1)

        for stone in stones[1:]:
            steps = usedSteps[stone]
            for step in steps:
                for dx in [-1, 0, 1]:
                    nextStone = stone + step + dx
                    if nextStone > stone and nextStone in stoneSet:
                       usedSteps[nextStone].add(step + dx)

        return len(usedSteps[stones[-1]]) > 0

        
# @lc code=end
s = Solution()
res = s.canCross([0,1,3,5,6,8,12,17])
print(res)
